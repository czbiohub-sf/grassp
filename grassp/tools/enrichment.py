...
def calculate_cluster_enrichment(
    ...
    *Enrichr* analysis via ``gseapy`` using the list of proteins (genes)
    ...
    metric_key = f"{obs_key_added}_{enrichment_ranking_metric}"
    data.obs[obs_key_added] = groups[cluster_key].transform(
    ...
    if sort_ascending:
        data.obs.loc[data.obs[metric_key] >= enrichment_threshold, obs_key_added] = np.nan
    else:
        data.obs.loc[data.obs[metric_key] <= enrichment_threshold, obs_key_added] = np.nan
...
def enrichment_to_cluster_distribution(
    ...
    over the neighbour graph (:func:`~grassp.tl.competitive_diffusion`) it produces
    ...
    :func:`~grassp.tl.competitive_diffusion` (see
    ...
    categories : list of str
        ...
        :func:`~grassp.tl.competitive_diffusion`.
    """
